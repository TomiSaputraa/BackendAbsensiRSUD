/*
  ...
  - The primary key for the `sakit` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - You are about to alter the column `id_sakit` on the `sakit` table. The data in that column could be lost. The data in that column will be cast from `VarChar(7)` to `Int`.
...
-- AlterTable
ALTER TABLE `sakit` DROP PRIMARY KEY,
    MODIFY `id_sakit` INTEGER NOT NULL AUTO_INCREMENT,
    MODIFY `tanggal_input_sakit` DATETIME NOT NULL,
