/*
  ...
  - You are about to alter the column `status_cuti` on the `cuti` table. The data in that column could be lost. The data in that column will be cast from `Enum(EnumId(2))` to `Enum(EnumId(5))`.
...
ALTER TABLE `cuti` MODIFY `tanggal_input_cuti` DATETIME NULL DEFAULT CURRENT_TIMESTAMP(3),
    MODIFY `status_cuti` ENUM('Pending', 'Diterima', 'Ditolak') NULL DEFAULT 'Pending';
