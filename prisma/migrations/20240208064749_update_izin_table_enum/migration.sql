/*
  ...
  - You are about to alter the column `status_izin` on the `izin` table. The data in that column could be lost. The data in that column will be cast from `Enum(EnumId(2))` to `Enum(EnumId(4))`.
...
ALTER TABLE `izin` MODIFY `tanggal_mulai_izin` DATETIME NOT NULL,
    ...
    MODIFY `status_izin` ENUM('Pending', 'Diterima', 'Ditolak') NOT NULL DEFAULT 'Pending';
