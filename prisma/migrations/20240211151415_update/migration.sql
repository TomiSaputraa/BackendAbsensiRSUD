/*
  ...
  - You are about to alter the column `status_sakit` on the `sakit` table. The data in that column could be lost. The data in that column will be cast from `Enum(EnumId(4))` to `Enum(EnumId(6))`.
...
ALTER TABLE `sakit` MODIFY `tanggal_input_sakit` DATETIME NULL,
    MODIFY `status_sakit` ENUM('Pending', 'Diterima', 'Ditolak') NULL DEFAULT 'Pending';
